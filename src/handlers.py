...
async def start(message: Message):
    ...
    await message.answer(text, reply_markup=get_homescreen_keyboard())


async def new_pill(message: Message, state: FSMContext):
    await state.set_state(Dialog.new_pill_title)
    await message.answer('Я готов добавить новый препарат, теперь введи название.', reply_markup=get_cancel_keyboard())
...
async def pill_title(message: Message, state: FSMContext):
    ...
    text = f'Отлично, я запомнил препарат {message.text}. Теперь введи время в формате HH:MM, например 08:30'
...
async def pill_time(message: Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=get_save_pill_keyboards())
...
async def all_pills(message: Message):
    ...
    await message.answer(text, reply_markup=get_all_pills_keyboards())


async def cancel(message: Message, state: FSMContext):
    if await state.get_state() is None:
        return await message.answer('Ты уже на главной странице.', reply_markup=get_homescreen_keyboard())
    await state.finish()
    await message.answer('Действие отменено. Ты снова на главной странице.', reply_markup=get_homescreen_keyboard())
